...
TIERS: dict[SubscriptionTier, TierConfig] = {
    ...
    SubscriptionTier.MASTER: TierConfig(
        ...
        monthly_quota=300,
        description=(
            "300 разблокировок в месяц и свои вопросы к раскладам — "
            "тариф для тех, кто раскладывает каждый день и не хочет "
            "думать об остатке."
        ),
        badge="Максимум",
        perks=(
...
# Чата поддержки как перка тарифа больше нет: поддержка одна и доступна
# всем из профиля (frontend/src/content/contacts.ts). Продавать «личный
# чат» за 1199 руб., когда тот же адрес открыт каждому бесплатно, значит
# обещать несуществующее отличие.
