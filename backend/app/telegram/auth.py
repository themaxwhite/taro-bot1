...
def validate_login_widget(
    ...
    """
    Проверяет данные виджета Telegram Login — того, что стоит на входе в
    админ-панель:
    https://core.telegram.org/widgets/login#checking-authorization

    Схема похожа на initData мини-приложения, но ключ считается иначе:
    здесь секрет это SHA-256 от токена бота, а в initData — HMAC от строки
    "WebAppData" с токеном в качестве ключа. Перепутать их легко, и тогда
    подпись не сойдётся ни при каких данных.

    Строка для подписи собирается из всех пришедших полей, кроме hash,
    отсортированных по имени. Складываем то, что реально прислали, а не
    заранее известный набор: Telegram может добавить поле, и жёсткий
    список молча ломал бы вход.
    """
    if not payload:
        raise TelegramAuthError("Missing Telegram login data")

    received_hash = payload.get("hash")
    if not received_hash:
        raise TelegramAuthError("Login data is missing 'hash'")

    pairs = sorted((k, v) for k, v in payload.items() if k != "hash")
    ...
    if not hmac.compare_digest(computed_hash, str(received_hash)):
        raise TelegramAuthError("Invalid login data signature")

    auth_date_raw = str(payload.get("auth_date", ""))
    if not auth_date_raw.isdigit():
        raise TelegramAuthError("Login data is missing a valid 'auth_date'")

    # Подписанные данные здесь работают как пропуск: панель хранит их и
    # прикладывает к каждому запросу. Поэтому срок жизни ограничен —
    # иначе утёкший однажды пропуск действовал бы вечно.
    if time.time() - int(auth_date_raw) > max_age_seconds:
        raise TelegramAuthError("Login data has expired")

    if "id" not in payload or "first_name" not in payload:
        raise TelegramAuthError("Login data is missing required keys")
