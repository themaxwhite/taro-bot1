...
def confirm_payment(
    ...
    **Перед нажатием сверься с кабинетом Робокассы.** Панель не знает,
    ...
    # Номер счёта в конце — по той же причине, что и при автоматическом
    # зачёте: колонка уникальна, и без него второй ручной зачёт тем же
    # администратором упал бы на нарушении уникальности.
    payment.provider_payment_id = f"manual:{admin.telegram_id}:{payment.id}"
    try:
